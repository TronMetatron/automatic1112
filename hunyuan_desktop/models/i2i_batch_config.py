...
class I2IBatchConfig:
    ...
    global_images: List[str] = field(default_factory=list)  # max 2 paths
    ...
    def preview_text(self) -> str:
        ...
        return f"{total} images total ({' x '.join(parts)}) | {img_info} | Mode: {bot_display}"
    ...
    def parse_prompt_lines(raw_text: str, global_images: List[str]) -> tuple:
        ...
        for line in raw_text.strip().split("\n"):
            ...
            if matches:
                overrides[idx] = [m.strip() for m in matches[:2]]  # max 2 images
